...
void loadTauFR_MC(FRMap& FR_Data,
                    const std::string& year,
                    const std::string& channel)
{
    TString fileName = Form(
        "/Volumes/Lexar/DCH_analysis/rederived_fakes/Final_tau_fake_rates_DY/DY_tau_fake_rates_%s_only.root",
        channel.c_str()
    );

    TFile* f = TFile::Open(fileName);
    if (!f || f->IsZombie()) {
        std::cerr << "ERROR: cannot open MC FR file "
                  << fileName << std::endl;
        exit(1);
    }

    TString hBarrelName = Form(
        "DY_FR_%s_only_%s_eta0to1p5",
        channel.c_str(), year.c_str()
    );
    TString hEndcapName = Form(
        "DY_FR_%s_only_%s_eta1p5to2p5",
        channel.c_str(), year.c_str()
    );
    ...
    if (!hBarrel || !hEndcap) {
        std::cerr << "ERROR: missing MC FR histograms:\n"
                  << "  " << hBarrelName << "\n"
...
double getTauFakeRate2D(double pt,
                        double eta,
                        const std::string& year,
                        FRMap2D& FR_Data2D)
{
...
void loadTauFR_Data2D(FRMap2D& FR_Data2D,
                      const std::string& year)
{
    TString fileName =
...
void DCH_run2_tauFR(string inYear="2016preVFP", string inCuts="tight", const char* ext="root") {
    (void)ext;
