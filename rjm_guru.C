...
std::tuple<std::vector<int>, double, double> get_legs(const std::array<TLorentzVector,4> &Lep_lab, TLorentzVector &MET_lab){//this gives valid lepton legs with neutrinos
    ...
    cout<<"Mll "<< (Lep_ref[0]+Lep_ref[1]).M() <<"\t"<< (Lep_ref[2]+Lep_ref[3]).M() <<endl;
    cout<<"DCH masses "<< (nu[0]+nu[1]+Lep_ref[0]+Lep_ref[1]).M() <<"\t"<< (nu[2]+nu[3]+Lep_ref[2]+Lep_ref[3]).M() <<endl<<endl;
    
    return {ileg, MDCH1, MDCH2};
}
