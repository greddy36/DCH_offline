...
void StackHist_sig() {
    ...
    for (auto& kv : files) {
		for (const auto& fname : kv.second) {
		    TFile* file = new TFile(("hist_MY/" + fname).c_str(), "READ");
		    if (!file || file->IsZombie()) continue;
	        open_files[kv.first].push_back(file);
	    }
	}
	TFile *ifile_D1 = new TFile("hist_MY/HppM500_2016.root","READ");
	...
	for (int i = 0; i < sizeof(hist_list)/sizeof(hist_list[0]); i++) {
		...
		if (name.find("_mll") < name.length()) {hist["signal"]->SetLineColor(4); hist["signal"]->SetFillColor(4);}
		else {hist["signal"]->SetLineColor(2); hist["signal"]->SetFillColor(2);}
		
		hist["signal"]->Scale(5);
		//hist["signal"]->Scale(h_bkg_tot
		//hist["signal"]->Scale();
		...
		hist["signal"]->GetXaxis()->SetTitle(const_cast<char*>(X_string.c_str()));
		...
		bkg_stack->GetXaxis()->SetTitle(const_cast<char*>(X_string.c_str()));
		...
		legend->AddEntry(hist["signal"], "5 x H^{++} 500 GeV", "lep");
		legend->SetBorderSize(0);
		legend->Draw();
		
		// Output
		std::string s =  "hist_MY/", s1, s2;
