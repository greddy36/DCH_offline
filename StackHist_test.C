...
void StackHist_test() {
	...
	std::string summary_type = "3lep", year = "2018";
...
"h_iso4v_ee","h_iso4v_em","h_iso4v_mm","h_iso4v_eee","h_iso4v_eem","h_iso4v_eet","h_iso4v_emt","h_iso4v_emm","h_iso4v_mmm","h_iso4v_mmt","h_iso4v_ett","h_iso4v_mtt","h_iso4v_eeee","h_iso4v_eeem","h_iso4v_eemm","h_iso4v_mmem","h_iso4v_mmmm","h_iso4v_eeet","h_iso4v_eemt","h_iso4v_eett","h_iso4v_emtt","h_iso4v_emmt","h_iso4v_ettt","h_iso4v_mmmt","h_iso4v_mmtt","h_iso4v_mttt",
	...
	for (int i = 0; i < sizeof(hist_list)/sizeof(hist_list[0]); i++) {
		THStack* bkg_stack = new THStack("bkg_stack", "Data/MC;;Events");
		...
		for (auto& kv : files) {
		    for (const auto& fname : kv.second) {
		        ...
		        TH1D* h = dynamic_cast<TH1D*>(file->Get(hist_list[i]));
		        if (!h) continue;
		        h->Rebin(5);
		        if (!hist[kv.first]) {
		            hist[kv.first] = (TH1D*)h->Clone();
		            hist[kv.first]->Reset();
		            hist[kv.first]->Sumw2();
		        }
		        hist[kv.first]->Add(h);
		...
		for (int j = 0; j < bins; ++i) {
			h_mc_uncert_band->SetBinContent(j+1, 1.0);  // center at ratio = 1
			//h_mc_uncert_band->SetBinError(j+1, sqrt(0)/h_bkg_total->GetBinContent(j+1));
		} 
