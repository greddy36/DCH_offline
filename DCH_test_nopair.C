...
void createHistograms(std::vector<TH1F*>& histograms, const std::string& prefix, const std::string& label, int bins, double low, double high) {
	int nCh = 15;
    for (int i = 0; i <= nCh; i++) {
...
void scaleAndWriteHistograms(std::vector<TH1F*>& histograms, double xs_weight) {
...
void DCH_test_nopair(const char* ext = "root"){
	...
	for(int j = 0; j < nfiles; j++){
		...
		if (fname.find("ZHToTauTau") > fname.length()) continue;
