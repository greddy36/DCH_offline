...
void roofit_wz(){
	...
	double other_uncert = sqrt(tot_uncert_quadr["other"]+tot_uncert_quadr["DY"]+tot_uncert_quadr["ZZ"]+tot_uncert_quadr["VVV"]+tot_uncert_quadr["ttV"]+tot_uncert_quadr["WJ"]+tot_uncert_quadr["ST"]+tot_uncert_quadr["TTbar"])/h_other_bkg->Integral();
...
RooExtendPdf other_ext("other_ext", "Other Extended PDF", other_pdf, other_norm_constrained);
	...
	//VV component
	RooDataHist wz_hist("wz_hist", "VV", x, Import(*h_bkg_group["VV"]));
	RooHistPdf wz_pdf("wz_pdf", "VV PDF", x, wz_hist);
	RooRealVar wz_norm("wz_norm", "VV yield", h_bkg_group["VV"]->Integral(), 0.0, 10.0 * h_bkg_group["VV"]->Integral());
	RooExtendPdf wz_ext("wz_ext", "VV Extended", wz_pdf, wz_norm);
	...
	model.plotOn(frame, Components("wz_ext"), LineColor(3));
