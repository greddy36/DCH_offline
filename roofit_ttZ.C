...
void roofit_ttZ(){
    ...
    for (auto& kv : files) {
        for (const auto& fname : kv.second) {
			TFile* file = new TFile(("hist_CR_ttZ/" + fname).c_str(), "READ");
	...
	h_other_bkg->Add(h_bkg_group["ZH"]);
	...
	double other_uncert = sqrt(tot_uncert_quadr["other"]+tot_uncert_quadr["DY"]+tot_uncert_quadr["ZZ"]+tot_uncert_quadr["VV"]+tot_uncert_quadr["VVV"]+tot_uncert_quadr["ttW"]+tot_uncert_quadr["ZH"]+tot_uncert_quadr["WJ"]+tot_uncert_quadr["ST"]+tot_uncert_quadr["TTbar"])/h_other_bkg->Integral();
...
RooExtendPdf other_ext("other_ext", "Other Extended PDF", other_pdf, other_norm_constrained);
	...
	RooAddPdf model_core("model_core", "Total Model without constraints", RooArgList(ttz_ext, other_ext));
	...
	model.plotOn(frame, Components("ttz_ext"), LineColor(3));
	model.plotOn(frame, Components("other_ext"), LineColor(2));
	
	frame->Draw();
	double ttz_fitted = ttz_norm.getVal();
	double ttz_nominal = h_bkg_group["ttZ"]->Integral();
	double ttz_fit_err = ttz_norm.getError(); 
	double scale_factor_ttZ = ttz_fitted / ttz_nominal;
	double sf_err = ttz_fit_err / ttz_nominal;

	std::cout << "ttZ scale factor = " << scale_factor_ttZ <<"+-"<< sf_err<< std::endl;
